...
import { IconCircle, IconStopwatch, IconCheckCircle } from './Icons.tsx';
...
const RefillStatusTracker: React.FC<{ status: ManualRefillStatus }> = ({ status }) => {
  const L_PER_GALLON = 20; // As per user example: 3 gallons = 60L

  const getCompletedDetails = () => {
    if (status.amount) {
      const gallons = status.amount / L_PER_GALLON;
      const gallonsDisplay = gallons % 1 === 0 ? gallons.toFixed(0) : gallons.toFixed(1);
      return `${gallonsDisplay} Gallons (${status.amount}L) Added`;
    }
    return null;
  };

  const stageConfig = [
    { name: 'requested', title: 'Refill Requested', icon: <IconCircle className="w-5 h-5" />, time: status.requestedAt },
    { name: 'in-progress', title: 'Refill In Progress', icon: <IconStopwatch className="w-5 h-5" />, time: status.inProgressAt },
    { name: 'completed', title: 'Refill Completed', icon: <IconCheckCircle className="w-5 h-5" />, time: status.completedAt, details: getCompletedDetails() },
  ];
  ...
  return (
    <div className="p-4 bg-gray-50 rounded-xl">
      <h3 className="font-bold text-lg mb-4 text-gray-800">Refill Status</h3>
      <div>
...
export default RefillStatusTracker;
